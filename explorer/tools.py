...
def initialize_data_set(state: DataAnalysisState, config: RunnableConfig) -> DataAnalysisState:
    ...
    state['dataset_description'] = initial_context
...
def explore_data_set(state: DataAnalysisState, config: RunnableConfig) -> DataAnalysisState:
    ...
    # make query plan
        ...
        start = True if state['messages'] == [] else False
        system_message = SystemMessage(content=exploration_prompt.format(initial_description=initial_context, start=start))
        # human_prompt = """Here is the dataset description so far:
        # <dataset_description>
        # {dataset_description}
        # </dataset_description>
        # """
        if start:
            human_message = HumanMessage(content="Explore the dataset")
            updated_messages = [system_message] + [human_message]
            state['messages'] = add_messages(state['messages'], updated_messages)

        response = gemini.bind_tools([Query, Finish], tool_choice="any").invoke(state['messages'])
...
def execute_exploration(state: DataAnalysisState, config: RunnableConfig) -> DataAnalysisState:
    ...
    system_message = SystemMessage(content=execute_exploration_prompt.format(dataset_description=initial_context))
    human_prompt = (
